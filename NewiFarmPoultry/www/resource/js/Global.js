...
//    this.AppPath = "http://vm-cpt-devslweb.cpf.co.th/SsISwineDeviceWeb/client/assets/www/";
    ...
    /*
    this.RootUrl = "http://192.168.101.103/SsIPoultryPrd/";
    this.SvcUrl = "http://192.168.101.103/SsIPoultryPrd/SsIPoultryDeviceService.svc";
        this.ReportUrl = "http://192.168.101.103/SsIPoultryPrd/SsIFarmReportService.svc";
        this.AppPath = "http://192.168.101.103/SsIPoultryPrd/client/assets/www/";
    */
...
//noppol.von 02/07/2013  read direct to HH_UI_LANG to get Desc
GlobalVar.prototype.RenderFooter = function (pageName, footerId) {
    // $.Util.RenderUiLang('iFarm' ) ;
    var cmd = $.Ctx.DbConn.createSelectCommand();
    cmd.sqlText = "select * from HH_UI_LANG where PROGRAM_ID = ? AND FORM_ID = ?";
    cmd.parameters.push($.Ctx.ProgramId);
    cmd.parameters.push('iFarm');
    
    cmd.executeReader(function (tx, res) {
                      var arrLang = new Array();
                      if (res.rows.length != 0) {
                      for (var idx = 0; idx < res.rows.length; idx++)
                      arrLang.push(res.rows.item(idx));
                      }
                      // alert($.Ctx.Lcl('iFarm', 'MsgOk', 'not found'));
                      //    alert( $.Ctx.Nvl(_.where(arrLang, {OBJECT_ID : 'MsgOk'})[0].TEXT_ENG , 'Not found' ) );
                      var textLang = 'TEXT_ENG';
                      if ($.Ctx.Lang != 'en-US')
                      textLang = 'TEXT_LOCAL';
                      ...
                      var msgSetting = 'Setting';
                      if (_.where(arrLang, { OBJECT_ID: 'msgSetting' }).length > 0)
                      msgSetting = eval("_.where(arrLang, { OBJECT_ID: 'msgSetting' })[0]." + textLang);
                      
                      s += '<span id="lblFooterSetting" data-lang ="msgSetting"  style="padding:0px">{0}</span></a></td><td  style="padding: 0px"> '.format([msgSetting]);
                      s += ' <a  data-inline="true"  data-role="button"  data-tag="footerbutton"  data-icon="home" style="padding: 0px;margin-left: 0px; margin-right: 0px;margin-top: 0px; margin-bottom: 0px;" onclick="$.Ctx.NavigateHome()">';
                      var msgHome = 'Home';
                      if (_.where(arrLang, { OBJECT_ID: 'msgHome' }).length > 0)
                      msgHome = eval("_.where(arrLang, { OBJECT_ID: 'msgHome' })[0]." + textLang);
                      
                      s += '<span id="lblFooterHome"  style="padding:0px">{0}</span></a></td></tr></table>'.format([msgHome]);
                      s += '  </td></tr></table>';
                      
                      if ((footerId != undefined) && (footerId != null)) {
                      $('#' + pageName + ' #' + footerId).html(s);
                      $('#' + pageName + ' #' + footerId).trigger('create');
                      } else {
                      $('#' + pageName + ' div[data-role="footer"]').html(s);
                      $('#' + pageName + ' div[data-role="footer"]').trigger('create');
                      }
                      });
...
GlobalVar.prototype.MsgBox = function (message, alertCallback, title, buttonName) {
    title = title == undefined ? $.Ctx.Lcl('iFarm', 'MsgAlert', "Information") : title;
