...
FarmVar.prototype.Poultry_CalculateAmountOfQuantityByRefDoc = function(farmorg, productcode, stocktype, SuccessCB) {
    var cmd = $.Ctx.DbConn.createSelectCommand();
    cmd.sqlText = "select sum(CASE WHEN m.[USED] = 0 then ifnull(m.qty,0) else ifnull(m.qty, 0)*-1 end) as QTY_SUM from hh_fr_ms_material_issued m where M.STOCK_TYPE = ? and M.PRODUCT_CODE = ? and m.ORG_CODE = ? and m.used is not 1 group by m.PRODUCT_CODE"
...
FarmVar.prototype.setS1StockCommand = function (farmOrgLoc, lastFarmOrgLoc, newProductCode, oldProductCode, mode, newWgh, newQty, oldWgh, oldQty, cmds, transType, SuccessCB, FailCB) {
    if ($.Ctx.Nvl(farmOrgLoc, '') == '' &&
        $.Ctx.Nvl(newProductCode, '') == '') {
        // no command to gen return lastest command
        SuccessCB(cmds);
        return false;
    }
    ...
    cmd.parameters.push(stkBalcObj.RECEIVED_DATE);
    cmd.executeReader(function (t, res) {
        console.log('Farm ', farmOrgLoc, 'Mode ', mode, 'Found ', res.rows.length);
        if (res.rows.length != 0) {
            ...
            if (mode == 'Create') {
                ...
                if (oldProductCode != '' && oldProductCode != newProductCode) {
                    ...
                    if (transType == '2') {
                        ...
                        // Type 1 must minus oldData to get new Data 
                        if (lastFarmOrgLoc != '') {
                        //case when move to farmInHusbandry plus only newQty
                            stkBalcObj.QUANTITY = res.rows.item(0).QUANTITY  + newQty;
                            stkBalcObj.WEIGHT = res.rows.item(0).WEIGHT + newWgh;
                        }
                        else {
                            stkBalcObj.QUANTITY = res.rows.item(0).QUANTITY - oldQty + newQty;
                            stkBalcObj.WEIGHT = res.rows.item(0).WEIGHT - oldWgh + newWgh;
                        }
                    }
                }
            }
            ...
            if (callBackNow) {
                var iCmdBalance = stkBalcObj.updateCommand($.Ctx.DbConn);
                cmds.push(iCmdBalance);
                if (stkBalcObj.QUANTITY == 0 && stkBalcObj.WEIGHT == 0)
                    cmds.push(stkBalcObj.deleteCommand($.Ctx.DbConn));
                if (typeof SuccessCB == 'function') SuccessCB(cmds);
            }
            ...
            if (mode != 'Delete') {
                ...
                stkBalcObj.WAREHOUSE_CODE = farmOrgLoc;
                stkBalcObj.PRODUCT_CODE = newProductCode;
                stkBalcObj.FUNCTION = 'A';
                stkBalcObj.QUANTITY = newQty;
                stkBalcObj.WEIGHT = newWgh;
                cmds.push(stkBalcObj.insertCommand($.Ctx.DbConn));
            }
            if (typeof SuccessCB == 'function') SuccessCB(cmds);
        }
    }, function (error) {
