...
$('#issue_stock_main').bind('pageinit', function (e,ui) {
    var self = $('#issue_stock_main');
    console.log('issue_stock_main pageinit');

    //mockup for debug
    var docType ={DOC_TYPE1:'DCTYP92',TRN_CODE1:'21'}
    //var docType ={DOC_TYPE1:'DCTYP66',TRN_CODE1:'21',DOC_TYPE2:'DCTYP61',TRN_CODE2:'11'}
    $.Ctx.SetPageParam('issue_stock_main','param',docType);


    var docParam = $.Ctx.GetPageParam('issue_stock_main','param');
    ...
    $('#issue_stock_main #lnkStockMainBack').click(function (e) {
        $.Ctx.NavigatePage($.Ctx.GetPageParam('issue_stock_main', 'Previous'), null, { transition: 'slide', reverse: 'true' });
    });
...
$('#issue_stock_main').bind('pagebeforecreate', function (e) {
    $.Util.RenderUiLang('issue_stock_main');
    $.Ctx.RenderFooter('issue_stock_main');
});

$('#issue_stock_main').bind('pagebeforehide', function (e) {
    $.StockCtx.PersistPageParam();
});

$('#issue_stock_main').bind('pagebeforeshow', function (e) {
    var self = $('#issue_stock_main');
...
function populateStockTranListView(stockTrns) {
    var lst = $('#issue_stock_main #stock-doc-list');
    lst.empty();
    ...
    $.Ctx.SetPageParam('issue_stock_main','stockTrans',stockTrns);
    lst.listview('refresh');

    //register click event
    $('#issue_stock_main #stock-doc-list li a').bind('click', function (e) {
        e.preventDefault();

        $.StockCtx.StockMode = 'EDIT';
        var idx =$(this).attr('id');
        var stockTrans =$.Ctx.GetPageParam('issue_stock_main','stockTrans');
        ...
        if($.StockCtx.param.length ==1){
            $.StockCtx.getStockDetail($.StockCtx.param[0].DOC_TYPE,$.StockCtx.DocDate,stockTrans[idx].DOC_NUMBER,function (stock) {
                ...
                var trnCtl = $.StockCtx.StockTranControl;
                if (trnCtl[0].CONF_HEADER == 'Y') {
                    $.Ctx.NavigatePage('stock_doc_header', { Previous: 'issue_stock_main', Action: 'View' }, { transition: 'slide' });
                }
                else {
                    if (trnCtl[0].CONF_DETAIL == 'Y') {
                        $.Ctx.NavigatePage('stock_doc_detail', { Previous:'issue_stock_main', Action: 'View' }, { transition: 'slide' });
                    }
                }
            });
        }else{
