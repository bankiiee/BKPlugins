...
(function () {
    ...
    require = function (id) {
        ...
        } else if (id in inProgreSsIPoultrydules) {
            var cycle = requireStack.slice(inProgreSsIPoultrydules[id]).join('->') + '->' + id;
            throw "Cycle in require graph: " + cycle;
        }
        if (modules[id].factory) {
            try {
                inProgreSsIPoultrydules[id] = requireStack.length;
                requireStack.push(id);
                return build(modules[id]);
            } finally {
                delete inProgreSsIPoultrydules[id];
                requireStack.pop();
            }
        }
